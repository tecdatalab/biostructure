...
def get_total_neighbors_of_neighbors(G, node,
...
def compute_aligning_costs(G1, G2, g1_nodes_with_data, g2_nodes_with_data):
    ...
    C = np.zeros((G1.number_of_nodes(),
                  G2.number_of_nodes()))  # Crear matrix n(cantidad de aristas g1) x m(cantidad de aristas g2)
    Z = np.zeros((G1.number_of_nodes(), G2.number_of_nodes()))

    for i in range(G1.number_of_nodes()):
        for j in range(
                G2.number_of_nodes()):  # Para todos los nodos calcular su similidad (Z) y potencial de convertirse en raiz (C)

            nodes_str_simi = 1.0 - float(g1_degree[i][1] + g2_degree[j][1]) / float(max_degree_G1 + max_degree_G2)
            total_str_simi = float(
                structural_similarity_node(G1, G2, g1_degree, g2_degree, g1_degree[i][0], g2_degree[j][0]))
...
def ok_neighbors(G1, G2, g1_mapping, g2_mapping, node1, node2,
...
def compute_neighbors_matriz(G1, G2, g1_mapping, g2_mapping, total_g1_neighbors, total_g2_neighbors,
    ...
    for i in range(len(total_g1_neighbors)):
        for j in range(len(total_g2_neighbors)):
            N[i][j] = ok_neighbors(G1, G2, g1_mapping, g2_mapping, total_g1_neighbors[i], total_g1_neighbors[j],
                                   equivalences_list)
...
def graph_aligning(G1, G2, min_similarity_value, repair_external_values=True):
    ...
    C, Z = compute_aligning_costs(G1, G2, g1_nodes_with_data,
    ...
    while most_similarity_value <= min_similarity_value:
        ...
        g1_neighbors, g2_neighbors, queue_neighbors = get_promising_neighbors(G1, G2, g1_dic, g2_dic, g1_mapping,
        ...
        while queue_neighbors:  # Crecemos en ancho para cada un de los vecinos
            ...
            g1_neighbors, g2_neighbors, new_queue_neighbors = get_promising_neighbors(G1, G2, g1_dic, g2_dic,
