...
def test_1_generate_descriptor_file():
    ...
    temp = filecmp.cmp("temp/contour.inv",
                       "{0}temp/contour.inv".format(valg.dir))
    assert temp
...
def test_2_generate_descriptor_file_for_union():
    ...
    valg.generate_descriptor_file_for_union(
        "0001", [0.018, one_third_contour], "one_third_contour")
    temp = filecmp.cmp("temp/one_third_contour.inv",
                       "{0}temp/one_third_contour.inv".format(valg.dir))
    assert temp
...
def test_3_generate_descriptors_files():
    ...
    temp_0 = filecmp.cmp("temp/contour.inv",
                         "{0}temp/contour.inv".format(valg.dir))
    temp_1 = filecmp.cmp("temp/one_third_contour.inv",
                         "{0}temp/one_third_contour.inv".format(valg.dir))
    temp_2 = filecmp.cmp("temp/two_thirds_contour.inv",
                         "{0}temp/two_thirds_contour.inv".format(valg.dir))
    temp_3 = filecmp.cmp("temp/one_third_two_thirds_contour.inv",
                         "{0}temp/one_third_two_thirds_contour.inv".format(valg.dir))
    temp_4 = filecmp.cmp("temp/one_std_contour.inv",
                         "{0}temp/one_std_contour.inv".format(valg.dir))
