...
def update_emd(
    ...
    if mode == 'c':
        ...
        emds = conec_ftp.get_emds_higher_than_date(
            conec_sql.last_update(), initialEMD, finalEMD)
    ...
    for i in emds:
        ...
        temp_emd_entry.emd_url = "http://{0}{1}".format(
            emd_url_server, emd_url_path)
        temp_emd_entry.create_by_ftp(i, conec_ftp.ftp)
        temp_time_stamp = Time_stamp(
            i,
            date.today(),
            temp_emd_entry.map_url,
            temp_emd_entry.xml_url,
            temp_emd_entry.image_url)
        ...
        if image == 'Y':
            try:
                imagpath, gifpath = gifG.generateGif(
                    i, temp_emd_entry.map_countour_level)
            except Exception as e:
                log_file.write("+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n")
                log_file.write(
                    "Error in the images generation of EMD {0}".format(i) + "\n")
        ...
        if len(map_result) == 1:
            ...
            try:
                ...
                log_file.write("+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n")
                log_file.write(
                    "Error in the update of EMD {0}".format(i) + "\n")
                log_file.write(str(e) + "\n")
                log_file.write("+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n")
            ...
            if descriptor == 'Y':
                if temp_emd_entry.map_countour_level is not None and temp_emd_entry.map_std is not None:
                    try:
                        ...
                        log_file.write("+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n")
                        log_file.write(
                            "Error in the descriptor generation of EMD {0}".format(i) + "\n")
                        log_file.write(str(e) + "\n")
                        log_file.write("+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n")
            ...
            try:
                ...
                log_file.write("+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n")
                log_file.write(
                    "Error in the insert of EMD {0}".format(i) + "\n")
                log_file.write(str(e) + "\n")
                log_file.write("+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\n")
        ...
        conec_sql.commit()
        k += 1
    if (conec_sql.last_update() is None):
...
def main(
    ...
    conec_ftp.init_connection(
        initial_directory="{0}{1}".format(
            emd_url_path, "/structures/"))
