...
def test_1_get_all_emds_id(ftpserver):
    ...
    copy_files(ftpserver)
    ftp_connection.init_connection("structures")
    result = ftp_connection.get_emds_higher_than_date(datetime.datetime(2000,2,14),"0001","inf")
    assert result == ["0001"]
...
def test_2_get_all_emds_id(ftpserver):
    ...
    copy_files(ftpserver)
    ftp_connection.init_connection("structures")
    result = ftp_connection.get_emds_higher_than_date(datetime.datetime(2020,2,14),"0001","inf")
    assert result == []