...
class Visualizer():
    ...
    def __init__(self, myMolecule, level=None):
        ...
        self.th_level = np.min(data) + level * (np.max(data) -
                                                np.min(data)) if level is not None else threshold_otsu(data)
    ...
    def show(self, export=False, start_angle=None, time=1, export_path=""):
        ...
        if self.labels.any() is not None:
            V = np.zeros(
                len(verts), [
                    ("position", np.float32, 3), ("normal", np.float32, 3), ("color", np.float32, 3)])
            colors = label2rgb(
                self.labels[verts_index[:, 0], verts_index[:, 1], verts_index[:, 2]])
            V["color"] = colors * 0.5
        else:
            V = np.zeros(
                len(verts), [
        ...
        if self.atoms is not None:
            ...
            amin, amax = np.min(
                atoms_coords["position"]), np.max(
                atoms_coords["position"])
            atoms_norm = 2 * \
                (atoms_coords["position"] - amin) / (amax - amin) - 1
            # Generalte atoms vertices

            slices = 32 + 1
            stacks = 32 + 1
            n = slices * stacks
            V_atoms = np.zeros(len(atoms_norm) * n,
                               [("position",
                                 np.float32,
                                 3),
                                ("normal",
                                ...
                                ("color",
                                 np.float32,
                                 3)])
            sphere_verts = np.zeros(
                n, [("position", np.float32, 3), ("normal", np.float32, 3), ("color", np.float32, 3)])
            radius = 0.03
            theta1 = np.repeat(
                np.linspace(
                    0,
            ...
            theta2 = np.tile(
                np.linspace(
                    ...
                    endpoint=True),
                stacks)
            sphere_verts["position"][:, 0] = np.sin(
                theta1) * np.cos(theta2) * radius
            sphere_verts["position"][:, 1] = np.cos(theta1) * radius
            sphere_verts["position"][:, 2] = np.sin(
                theta1) * np.sin(theta2) * radius
            sphere_verts = np.repeat(sphere_verts[None, :], len(
                atoms_norm), axis=0).reshape(-1)
            atoms_norm = np.repeat(atoms_norm, n, axis=0)
            V_atoms["position"][:, 0] = atoms_norm[:, 2] + \
                sphere_verts["position"][:, 2]
            V_atoms["position"][:, 1] = atoms_norm[:, 1] + \
                sphere_verts["position"][:, 1]
            V_atoms["position"][:, 2] = atoms_norm[:, 0] + \
                sphere_verts["position"][:, 0]
            V_atoms["normal"] = V_atoms["position"]
            V_atoms["color"] = np.repeat(
                np.array([[0.0, 0.1, 1.0]]), len(atoms_norm), axis=0)

            vamin, vamax = V_atoms["position"].min(), V_atoms["position"].max()
            V_atoms["position"] = 2 * \
                (V_atoms["position"] - vamin) / (vamax - vamin) - 1
            ...
            for atom in range(len(atoms_coords)):
                for i in range(stacks - 1):
                    for j in range(slices - 1):
                        ...
                        indices.append(
                            i * (slices) + j + slices + 1 + initial_offset)
                        indices.append(
                            i * (slices) + j + slices + initial_offset)
                        indices.append(
                            i * (slices) + j + slices + 1 + initial_offset)
                        indices.append(i * (slices) + j + initial_offset)
                initial_offset += n
            ...
            I = np.concatenate(
                (indices, (faces).astype(
                    np.uint32) + len(V_atoms)), axis=0)
        ...
        framebuffer = np.zeros(
            (window.height, window.width * 3), dtype=np.uint8)
    ...
    def add_structure(self, filename):
        ...
        self.atoms = np.zeros(
            len(atoms), [
                ("id", np.int32, 1), ("position", np.float32, 3)])
        self.atoms["id"] = atoms["atom_number"].values
        self.atoms["position"] = atoms[[
            "z_coord", "y_coord", "x_coord"]].values
        ...
        if self.labels is None:
            ...
            for atom in self.atoms:
                ...
                for region in regionprops(self.labels):
                    ...
                    min_box = 2 * \
                        (np.array([min_z, min_y, min_x]) - map_min) / (map_max - map_min) - 1
                    max_box = 2 * \
                        (np.array([max_z, max_y, max_x]) - map_min) / (map_max - map_min) - 1

                    coord = 2 * (atom["position"] - pdb_min) / \
                        (pdb_max - pdb_min) - 1
                    if np.all(coord <= max_box) and np.all(coord >= min_box):
                        atoms_labels[atom["id"]].append(region.label)
                        print(
                            "Found atom %d with coords (%f, %f, %f) in region %d " %
                            (atom["id"], coord[0], coord[1], coord[2], region.label))

    def segmentate(self, smooth=False, sigma=1.0):
        molecule = self.molecule
        if smooth:
            molecule = processing.gaussian_smooth(molecule, sigma)
        self.labels = processing.watershed_segmentation(
            molecule, self.th_level)
