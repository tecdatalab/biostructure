...
bool meanshift(MRC *m,POINTS *p){
 ...
 int cnt=0;
 int xydim=m->xdim*m->ydim;
 double ShiftCut2 = (cmd.AllowShift)*(cmd.AllowShift)/(m->widthx*m->widthx);
 ...
 for(int i=0;i<cnt;i++){
  ...
  while(1){
   ...
   if(tmpcd[0]*tmpcd[0]+tmpcd[1]*tmpcd[1]+tmpcd[2]*tmpcd[2]<0.001)
    break;
   //Shift Distance
   tmpcd[0]=p->cd[i][0]-pos2[0];
   tmpcd[1]=p->cd[i][1]-pos2[1];
   tmpcd[2]=p->cd[i][2]-pos2[2];
   if(tmpcd[0]*tmpcd[0]+tmpcd[1]*tmpcd[1]+tmpcd[2]*tmpcd[2]>ShiftCut2)
    break;
  }
  //printf("*%d %f %f %f\n",i,pos[0],pos[1],pos[2]);
  p->cd[i][0]=pos[0];
